    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          >
            <input className="input w-56" placeholder="本文を検索（3文字以上）" value={q} onChange={(e) => setQ(e.target.value)} />
          </form>
        </div>
      </div>
      <div className="card p-0">
        ...
        <ul className="divide-y divide-slate-100">
          {query.data?.map((c) => (
            <li key={c.id}>
              <Link to={`/inbox/${c.id}`} className={`flex items-start gap-3 px-4 py-3 hover:bg-slate-50 ${c.unread ? 'bg-blue-50/40' : ''}`}>
                <span className={`${channelBadge(c.channel)} mt-0.5 w-20 justify-center`}>{channelLabel(c.channel)}</span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`truncate ${c.unread ? 'font-bold' : 'font-medium'}`}>{c.client?.name ?? c.counterpartName ?? c.counterpartAddress ?? '（不明）'}</span>
                    {!c.clientId && <span className="badge badge-orange">未紐付け</span>}
                    {c.needsReply && <span className="badge badge-blue">要返信</span>}
                    {c.unread > 0 && <span className="badge badge-blue">{c.unread}</span>}
                    {c.subject && <span className="truncate text-xs text-slate-500">{c.subject}</span>}
                  </div>
                ...
                <span className="shrink-0 text-xs text-slate-400">{fmtRelative(c.lastMessageAt)}</span>
              </Link>
            </li>
          ))}
        </ul>
