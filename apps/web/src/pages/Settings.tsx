...
const FIELDS: { key: string; label: string; hint?: string; multiline?: boolean; options?: { value: string; label: string }[] }[] = [
  ...
  },
  {
    ...
    hint: '「自分宛だけ」にすると、[To] で自分が指定されたもの・自分のメッセージへの返信（re）・全員宛（toall）・ダイレクトチャット・自分に振られたタスクのメッセージだけを受信箱に取り込みます。引用（[qt]）の中の [To:自分] は自分宛と見ません。グループでの自分の発言は、取り込み済みのやり取りへの返信だけ入ります',
    options: [
  ...
  { key: 'business_hours_start', label: '営業開始（例: 10:00）', hint: '候補日の検索はこの時刻から。分まで指定できます' },
  { key: 'business_hours_end', label: '営業終了（例: 18:00）' },
