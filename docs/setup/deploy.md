...
ホーム画面・タスクバーのアイコンに、対応が要る件数を数字で出せます。出す数は 設定 →「アプリのアイコンに出す件数」で選びます。

| 選ぶもの | 出る数 |
|---|---|
| 受信箱の未読だけ | まだ**開いていない**会話の数。開くと消えます（メールの未読と同じ感覚） |
| 受信箱の未返信だけ（既定） | 相手から届いていて、まだ**返していない**会話の数。読んでいても、返すまで残ります |
| 受信箱の未返信＋要確認 | 上に「要確認」の件数を足したもの |
| 表示しない | 数字を出しません |

| 端末 | 条件 |
|---|---|
| iPhone / iPad | iOS・iPadOS 16.4 以降で、**Safari の「ホーム画面に追加」で入れたアプリ**のみ。さらに**通知の許可**が必要（設定画面の「通知を許可する」から） |
| Windows / Mac | Chrome / Edge で**インストールしたアプリ**で出ます（通知の許可は不要） |
| ブラウザのタブで開いているとき | 出ません（アプリとして入れたときだけ） |

アプリを開いている間（別ウィンドウで後ろに置いている間も含む）は、1 分ごとの更新と操作のたびに追随します。**アプリを閉じている間も、下の「受信したらすぐ知らせる（通知）」を有効にしていれば、連絡が届いたときに数字が更新されます。** 通知を切っている場合は、閉じている間は最後の数字のまま残ります。

### 受信したらすぐ知らせる（通知）

LINE・Chatwork・Gmail に連絡が届いたら、アプリを開いていなくても端末に通知を出します。相手の名前と本文の冒頭が出て、押すとその会話が開きます。

**設定の場所**: 設定 →「受信したらすぐ知らせる（通知）」

**使い始める手順**（端末ごとに 1 回ずつ）

1. 通知を受け取りたい端末で、アプリ（iPhone・iPad はホーム画面に追加したもの）を開く
2. 設定 →「受信したらすぐ知らせる（通知）」→「この端末で受け取る」を押す
3. 端末が出す許可の確認で「許可」を選ぶ
4. 「テスト送信」を押して、通知が届くことを確かめる

登録した端末は同じ画面に一覧で出ます。複数の端末（iPhone とパソコンなど）を登録すると、すべてに届きます。

| 端末 | 条件 |
|---|---|
| iPhone / iPad | iOS・iPadOS 16.4 以降で、**Safari の「ホーム画面に追加」で入れたアプリ**のみ。Safari のタブで開いているときは受け取れません |
| Windows / Mac | Chrome / Edge / Safari。ブラウザのタブのままでも受け取れます（ブラウザが起動している間） |

**届くまでの時間**

| チャネル | 速さ | 理由 |
|---|---|---|
| LINE公式 | 届いた瞬間（数秒） | Webhook で届いた時点でアプリに入るため |
| Chatwork | 届いた瞬間（数秒） | 同上。Webhook が止まっているときは 5 分ごとのポーリングで拾います |
| Gmail | 最大 1 分 | Gmail は毎分見に行く方式のため |

まとめて何件も届いたときは、通知が連発しないよう 4 秒ほどまとめて「新しい連絡 3 件」のように 1 通にします。

**通知を止めたいとき**

- すべての端末で止める … 設定 →「受信したらすぐ知らせる（通知）」の「受信したら通知する」のチェックを外して保存
- その端末だけ止める … 同じ画面の「受け取らない」を押す

**注意**

- 端末ごとに許可が必要です。新しい端末を使い始めたら、その端末でもう一度「この端末で受け取る」を押してください
- 一度「許可しない」を選ぶと、端末の「設定 → 通知」から許可し直す必要があります
- 通知には相手の名前と本文の冒頭 120 字が出ます。人に画面を見られる場所で使うときはご注意ください
...
**画面から復元する（推奨）**: 設定 → バックアップ の「復元」で、ダウンロードしておいた `.db.gz`（または `.db`）を選んで「復元」を押します。サーバー内に残っている世代は、一覧の「この世代に戻す」で戻せます。復元前の状態は自動でバックアップに残るので、間違えたときはそれに戻せます。

**ファイルを直接置き換える（画面が開けないとき）**:
