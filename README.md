# T-Lex（依頼者との連絡と事件の管理 / Legal Case Management）
...
| 添付の自動振り分け | 受信した画像・ファイルを OneDrive for Business の依頼者フォルダ（`受領資料/`）へ保存。依頼者不明のものは「未保存」として置き、必要なものだけ保存（設定で「すべて自動保存」「自動保存しない」にも変更可） |
...
| 予定 | 週・月・4 週間の一覧で予定を確認し、登録・編集・削除を Google カレンダーと同期。依頼者・事件・種別を紐付けられる |
...
推奨は Railway へのデプロイです（PC に何も入れず、公開 URL も自動。入力する変数はログイン用パスワード 1 つ）。手順は [docs/setup/deploy.md](docs/setup/deploy.md) の「A. Railway」を参照してください。Render.com でも同様に動きます。事務所 PC で動かす場合は次のとおりです。
